...
// } from "../services/Insightservice";   // ← FIXED: capital I matches the actual filename
...
// /* ─────────────────────────────────────────────────────────────────────────
//    toArr — handles every shape our backend can send:

//    Shape A (monthly profit / sales / top-selling):
//      { status, message, data: [...] }         → return data

//    Shape B (write-offs):
//      { status, message, data: { total_stock_value_lost, breakdown:[...] } }
//      → toArr returns [] here; we separately call extractWoBreakdown()

//    Shape C (direct arrays — some endpoints skip the wrapper):
//      [...]                                     → return as-is
//    ───────────────────────────────────────────────────────────────────────── */
// // success_response shape: axios res.data = {status, message, data: [...]}
// // So: res.data.data = the actual array
// const toArr = (res) => {
//   if (Array.isArray(res?.data?.data))  return res.data.data;   // standard success_response
//   if (Array.isArray(res?.data))        return res.data;         // direct array (no wrapper)
//   return [];
// };

// /* ─── write-off specific extractors ────────────────────────────────────── */
// // Write-off endpoint: success_response wraps as {status, message, data: {total_stock_value_lost, breakdown:[...]}}
// // Axios puts that envelope at res.data, so:
// //   res.data                        = {status, message, data: {...}}
// //   res.data.data                   = {total_stock_value_lost: X, breakdown: [...]}
// //   res.data.data.breakdown         = the array we need
// const extractWoBreakdown = (res) => {
//   // Primary: standard success_response shape
//   if (Array.isArray(res?.data?.data?.breakdown))  return res.data.data.breakdown;
//   // Fallback A: some versions skip the outer wrapper
//   if (Array.isArray(res?.data?.breakdown))         return res.data.breakdown;
//   // Fallback B: direct array
...
//   // Primary: res.data.data.total_stock_value_lost
//   if (res?.data?.data?.total_stock_value_lost != null)
//     return Math.abs(Number(res.data.data.total_stock_value_lost));
//   if (res?.data?.total_stock_value_lost != null)
//     return Math.abs(Number(res.data.total_stock_value_lost));
//   return null;  // null = sum from breakdown rows
// };
...
//                   {writeOffs.map((w, i) => {
//                     // backend field is stock_value_lost (from insights_router.py)
//                     const loss = Number(w.stock_value_lost || w.total_loss || w.loss_amount || 0);
//                     // backend field is total_written_off (formatted string like "0.50 pcs")
